...
async function buildPremiumCard(opts: PremiumCardOptions): Promise<HTMLCanvasElement> {
  ...
  ctx.textAlign = "center";
  fillTextTracked(ctx, "EVERY RIDE. A STORY.", W / 2, cursorY + FOOTER_H / 2 - s(30), s(3));

  ctx.font = `500 ${bf(9) * 2}px "${bodyFont}"`;
  ctx.fillStyle = "rgba(233,201,122,.5)";
  ctx.textAlign = "center";
  ctx.fillText("This card is generated by", W / 2, cursorY + FOOTER_H / 2 + s(14));
  ctx.fillText("www.theknightryders.com", W / 2, cursorY + FOOTER_H / 2 + s(38));
