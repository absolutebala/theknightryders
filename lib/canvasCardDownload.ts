...
async function buildPremiumCard(opts: PremiumCardOptions): Promise<HTMLCanvasElement> {
  ...
  ctx.textAlign = "center";
  fillTextTracked(ctx, "EVERY RIDE. A STORY.", W / 2, cursorY + FOOTER_H / 2 - s(10), s(3));

  ctx.font = `500 ${bf(9)}px "${bodyFont}"`;
  ctx.fillStyle = "rgba(233,201,122,.5)";
  fillTextTracked(ctx, "THIS CARD IS GENERATED BY WWW.THEKNIGHTRYDERS.COM", W / 2, cursorY + FOOTER_H / 2 + s(20), s(0.5));
